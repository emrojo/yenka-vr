...
AYenkaScenarioMenu::AYenkaScenarioMenu()
{
	...
	HeaderText->SetupAttachment(BackgroundPlate);
	HeaderText->SetRelativeLocation(FVector(-1.0f, 0.0f, 20.0f));
	...
	for (int32 i = 0; i < 6; ++i)
	{
		...
		Item.ButtonMesh->SetupAttachment(BackgroundPlate);
		if (CubeMesh)
		{
		...
		Item.TitleText->SetupAttachment(Item.ButtonMesh);
		Item.TitleText->SetRelativeLocation(FVector(-1.2f, 0.0f, 0.0f));
	...
	ReopenButtonText->SetupAttachment(ReopenButton);
	ReopenButtonText->SetRelativeLocation(FVector(-1.0f, 0.0f, 0.0f));
	...
	ReopenButtonText->SetWorldSize(2.2f);
...
void AYenkaScenarioMenu::BeginPlay()
{
	...
	SetActorLocation(FVector(-32.0f, 0.0f, 115.0f));
	SetActorRotation(FRotator(0.0f, 0.0f, 0.0f));

	// Initialize background material (Dark frosted translucent glass)
	...
	if (BaseMat)
	{
		...
		if (BgDynMat)
		{
			BgDynMat->SetVectorParameterValue(TEXT("Color"), FLinearColor(0.02f, 0.03f, 0.05f, 0.95f));
		...
		for (int32 i = 0; i < MenuItems.Num(); ++i)
		{
			if (MenuItems[i].ButtonMesh)
			{
				...
				if (CardMat)
				{
					// Dark charcoal base with slight theme tint
					FLinearColor BaseCardColor = FMath::Lerp(FLinearColor(0.08f, 0.09f, 0.11f, 1.0f), MenuItems[i].AccentColor, 0.20f);
...
void AYenkaScenarioMenu::Tick(float DeltaTime)
{
	...
	// Smooth scaling animation when opening / closing the main menu panel
	if (!FMath::IsNearlyEqual(CurrentScale, TargetScale, 0.01f))
	{
		...
		{
			if (BackgroundPlate) BackgroundPlate->SetVisibility(false, true);
...
void AYenkaScenarioMenu::ToggleMenuVisibility()
{
	...
	if (bIsMenuOpen)
	{
		if (BackgroundPlate) BackgroundPlate->SetVisibility(true, true);
...
void AYenkaScenarioMenu::ProcessRayHit(const FHitResult& HitResult, bool bIsClick)
{
	...
	if (HitIndex != -1)
	{
		...
		if (HoveredButtonIndex != HitIndex)
		{
			...
			{
				FLinearColor NormalColor = FMath::Lerp(FLinearColor(0.08f, 0.09f, 0.11f, 1.0f), MenuItems[HoveredButtonIndex].AccentColor, 0.20f);
