...
* **`[REQ-CROSS-21]` Strict Hand & Finger Physical Non-Penetration & Continuous Collision Solver:**
  * **Absolute Table & Jenga Board Clearance:** The hand avatar's entire anatomical volume (wrist, palm center, knuckles, and all 5 fingertips) is constrained to valid world space strictly above the tabletop and the Jenga board ($Z \ge \text{ActualTableZ} + 0.25\text{ cm}$ over the board, $Z \ge \text{ActualTableZ} - 2.0\text{ cm} + 0.25\text{ cm}$ over the desk). Any proposed gesture, camera tilt, or offset that would clip below the surfaces automatically elevates the hand vertically ($\Delta Z$).
